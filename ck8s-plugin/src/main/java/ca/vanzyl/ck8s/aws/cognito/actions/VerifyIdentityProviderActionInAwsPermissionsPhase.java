...
public class VerifyIdentityProviderActionInAwsPermissionsPhase extends CognitoTaskAction<CognitoTaskParams.CreateIdentityProviderParams> {

    private final CloudFormation cloudFormation;

    @Inject
    public VerifyIdentityProviderActionInAwsPermissionsPhase(CognitoClientFactory clientFactory, CloudFormation cloudFormation) {
        super(clientFactory);
    ...
    public TaskResult execute(Context context, CognitoTaskParams.CreateIdentityProviderParams input) throws Exception {
        ...
        cloudFormation.statement(new Statement(
                Statement.ALLOW,
                Set.of("cognito-idp:DescribeIdentityProvider"),
