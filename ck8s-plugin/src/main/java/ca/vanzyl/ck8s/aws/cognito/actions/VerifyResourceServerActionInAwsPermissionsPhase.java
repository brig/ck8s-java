...
public class VerifyResourceServerActionInAwsPermissionsPhase extends CognitoTaskAction<CognitoTaskParams.CreateResourceServerParams> {

    private final CloudFormation cloudFormation;

    @Inject
    public VerifyResourceServerActionInAwsPermissionsPhase(CognitoClientFactory clientFactory, CloudFormation cloudFormation) {
        super(clientFactory);
    ...
    public TaskResult execute(Context context, CognitoTaskParams.CreateResourceServerParams input) throws Exception {
        ...
        cloudFormation.statement(new Statement(
                Statement.ALLOW,
                Set.of("cognito-idp:DescribeResourceServer"),
